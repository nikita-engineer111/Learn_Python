...
def generate_access_config():
    config = []
    for interface in access_dict.keys():
        config.append(interface)
        for command in access_template:
            if command.endswith('access vlan'):
                config.append('%s %s' % (command, access_dict[interface]))
            else:
                config.append('%s ' % command)
    return config
...
def generate_access_config():
    ...
    for interface in access_dict.keys():
        ...
        for command in access_template:
            if command.endswith('access vlan'):
                config[interface].append('%s %s' % (command, access_dict[interface]))
            else:
